...
async function handleSetRuleset(interaction) {
  ...
  setWaitingForRuleset(interaction.guildId, interaction.user.id);
  await interaction.reply({
    content:
      "📝 Invia il tuo prossimo messaggio che vuoi salvare come ruleset.",
    ephemeral: false,
  });
  logger.info(
    `/setruleset used by ${interaction.user.tag} in ${interaction.guild.name}`,
  );
}
...
async function handleRuleset(interaction) {
  ...
  if (!config.ruleset_message) {
    await interaction.reply({
      content:
        "❌ Nessun ruleset configurato! Usa `/setruleset` per impostarne uno.",
      ephemeral: false,
    });
    return;
  }
  ...
  await interaction.reply({
    content: config.ruleset_message,
    ephemeral: false,
  });
  logger.info(
    `Ruleset shown to ${interaction.user.tag} in ${interaction.guild.name}`,
  );
}
