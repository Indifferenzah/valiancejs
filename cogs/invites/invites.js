...
const MESSAGES = {
  ERROR: {
    INTERNAL:
      "⚠️ Si è verificato un errore interno durante l'elaborazione della richiesta. Riprova più tardi.",
    NO_PERMISSION:
      "🚫 **Accesso Negato** — Non hai i permessi necessari per eseguire questo comando.",
    INVALID_PAGE: "📄 **Pagina Non Valida** — La pagina richiesta non esiste.",
    USER_NOT_FOUND:
      "👤 **Utente Non Trovato** — L'utente specificato non è stato trovato.",
    NO_DATA:
      "📊 **Nessun Dato Disponibile** — Non ci sono attualmente statistiche inviti da visualizzare.",
  },
  SUCCESS: {
    INVITE_SET:
      "✅ **Inviti Aggiornati** — Impostati con successo **{amount}** inviti per **{user}**.",
    INVITE_ADDED:
      "✅ **Inviti Aggiunti** — Aggiunti con successo **{amount}** inviti all'account di **{user}**.",
    INVITE_REMOVED:
      "✅ **Inviti Rimossi** — Rimossi con successo **{amount}** inviti dall'account di **{user}**.",
    CACHE_INITIALIZED:
      "Cache inviti inizializzata con successo con {count} inviti attivi.",
    MEMBER_TRACKED:
      "Membro {member} entrato tramite invito di {inviter} (Codice: {code}).",
  },
  INFO: {
...
class InviteTrackerCog {
      ...
      new SlashCommandBuilder()
          ...
          sc
            ...
            .addIntegerOption((o) =>
              o
                .setName("page")
                .setDescription(
                  "Numero di pagina da visualizzare (predefinito: 1)",
                )
                .setRequired(false)
                .setMinValue(1),
            ),
        ...
        .addSubcommand((sc) =>
          sc
            .setName("set")
            .setDescription(
              "⚙️ [ADMIN] Imposta il numero di inviti di un utente",
            )
            .addUserOption((o) =>
              o
                .setName("user")
        ...
        .addSubcommand((sc) =>
          sc
            .setName("add")
            .setDescription(
              "➕ [ADMIN] Aggiungi inviti al conteggio di un utente",
            )
            .addUserOption((o) =>
              o
                .setName("user")
        ...
        .addSubcommand((sc) =>
          sc
            .setName("remove")
            .setDescription(
              "➖ [ADMIN] Rimuovi inviti dal conteggio di un utente",
            )
            .addUserOption((o) =>
              o
                .setName("user")
  ...
  async _initialize() {
    ...
    if (!guild) {
      try {
        ...
        logger.error(
          `[${MODULE_NAME}] Failed to fetch guilds during initialization:`,
          error,
        );
      }
    }
    ...
    try {
      ...
      await this._cacheVanityInvite(guild);
  ...
  async _cacheVanityInvite(guild) {
    try {
      const vanity = await guild.fetchVanityData().catch(() => null);
      if (!vanity || !vanity.code) return;

      this.vanityCache.set(guild.id, {
        code: vanity.code,
        uses: vanity.uses || 0,
      });
    } catch (error) {
      logger.debug(
        `[${MODULE_NAME}] Vanity cache update skipped: ${error.message}`,
      );
    }
  }
  ...
  setUserInvites(userId, amount) {
    ...
    logger.info(
      `[${MODULE_NAME}] Set invites for user ${userId} to ${amount}.`,
    );
  }
  ...
  getLeaderboardData() {
    return Object.entries(this.db).sort((a, b) => b[1] - a[1]);
  }
  ...
  async handleUserCommand(interaction) {
    try {
      ...
      logger.info(
        `[${MODULE_NAME}] User ${interaction.user.tag} checked invites for ${targetUser.tag}.`,
      );
    } catch (error) {
  ...
  async handleLeaderboardCommand(interaction, page = 0) {
    try {
      ...
      const totalPages = Math.max(
        1,
        Math.ceil(leaderboardData.length / ITEMS_PER_PAGE),
      );
      ...
      logger.info(
        `[${MODULE_NAME}] Leaderboard displayed (Page ${page + 1}) for ${interaction.user.tag}.`,
      );
    } catch (error) {
      logger.error(
        `[${MODULE_NAME}] Error in handleLeaderboardCommand:`,
        error,
      );
  ...
  async handleAdminCommand(interaction, action) {
    try {
      if (
        ...
        return interaction.reply({
          content:
            this.config.messages.no_permission || MESSAGES.ERROR.NO_PERMISSION,
          ephemeral: true,
        });
      }
      ...
      switch (action) {
        case "set":
          ...
          responseMessage = MESSAGES.SUCCESS.INVITE_SET.replace(
            "{user}",
            targetUser.tag,
        ...
        case "add":
          ...
          responseMessage = MESSAGES.SUCCESS.INVITE_ADDED.replace(
            "{user}",
            targetUser.tag,
          ).replace("{amount}", amount);
          break;
        ...
        case "remove":
          this.modifyUserInvites(targetUser.id, -amount);
          responseMessage = MESSAGES.SUCCESS.INVITE_REMOVED.replace(
            "{user}",
            targetUser.tag,
          ).replace("{amount}", amount);
          break;
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields([
          ...
          {
            name: "📊 Conteggio Precedente",
            value: previousCount.toString(),
            inline: true,
          },
          {
            name: "📈 Nuovo Conteggio",
            value: newCount.toString(),
            inline: true,
          },
          {
            name: "🔄 Variazione",
            value: `${newCount >= previousCount ? "+" : ""}${newCount - previousCount}`,
            inline: true,
          },
        ])
  ...
  async handlePrefixLeaderboardCommand(message) {
    try {
      ...
      const leaderboardData = this.getLeaderboardData().slice(
        0,
        ITEMS_PER_PAGE,
      );
      ...
      logger.error(
        `[${MODULE_NAME}] Error in handlePrefixLeaderboardCommand:`,
        error,
      );
    }
  }
  ...
  async handleMemberJoin(member) {
    try {
      ...
      currentInvites.forEach((invite) => {
        ...
        const previousUses = cachedData?.uses ?? 0;
        const delta = (invite.uses || 0) - previousUses;

        if (delta > 0) {
          deltas.push({ invite, delta });
        }
      });
      ...
      if (!usedInvite) {
        ...
        if (vanity && vanity.code) {
          const previousVanity = this.vanityCache.get(guild.id);
          const previousUses = previousVanity?.uses ?? 0;

          if ((vanity.uses || 0) > previousUses) {
      ...
      if (inviter && usedInvite) {
        ...
        logger.info(
          `[${MODULE_NAME}] ${MESSAGES.SUCCESS.MEMBER_TRACKED.replace(
            "{member}",
            member.user.tag,
          )
      ...
      } else if (usedInvite && usedInvite.code) {
        logger.warn(
          `[${MODULE_NAME}] Member ${member.user.tag} joined via vanity invite (${usedInvite.code}).`,
        );
      } else {
        logger.warn(
          `[${MODULE_NAME}] Unable to determine invite source for ${member.user.tag}.`,
        );
      }
    } catch (error) {
...
function setup(client) {
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      if (
        ...
        interaction.customId.startsWith("inv_lb_")
      ) {
  ...
  client.on("messageCreate", async (message) => {
    try {
      ...
      const prefix = COMMAND_PREFIXES.find((p) =>
  ...
  logger.info(
    `[${MODULE_NAME}] Invite Tracking System initialized successfully.`,
  );
